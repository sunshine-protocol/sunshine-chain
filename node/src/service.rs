...
use sc_service::{
    error::Error as ServiceError, Configuration, RpcHandlers, ServiceComponents, TaskManager,
};
use sp_consensus_aura::sr25519::AuthorityPair as AuraPair;
use sp_inherents::InherentDataProviders;
use std::sync::Arc;
use std::time::Duration;
use sunshine_runtime::{self, opaque::Block, RuntimeApi};
...
pub fn new_full(config: Configuration) -> Result<(TaskManager, Arc<RpcHandlers>), ServiceError> {
...
pub fn new_light(config: Configuration) -> Result<(TaskManager, Arc<RpcHandlers>), ServiceError> {
