...
use sunshine_bounty_client::bounty::{
    BountyEventsDecoder, BountyPaymentExecutedEvent, BountyPostedEvent,
...
pub async fn bounty_post_subscriber(client: &Client) -> Result<EventSubscription<Runtime>> {
    let sub = client.chain_client().subscribe_events().await?;
    let mut decoder = EventsDecoder::<Runtime>::new(client.chain_client().metadata().clone());
    decoder.with_bounty();
...
pub async fn bounty_contribution_subscriber(client: &Client) -> Result<EventSubscription<Runtime>> {
    let sub = client.chain_client().subscribe_events().await?;
    let mut decoder = EventsDecoder::<Runtime>::new(client.chain_client().metadata().clone());
    decoder.with_bounty();
    let mut sub = EventSubscription::<Runtime>::new(sub, decoder);
...
pub async fn bounty_submission_subscriber(client: &Client) -> Result<EventSubscription<Runtime>> {
    let sub = client.chain_client().subscribe_events().await?;
    let mut decoder = EventsDecoder::<Runtime>::new(client.chain_client().metadata().clone());
    decoder.with_bounty();
    let mut sub = EventSubscription::<Runtime>::new(sub, decoder);
...
pub async fn bounty_approval_subscriber(client: &Client) -> Result<EventSubscription<Runtime>> {
    let sub = client.chain_client().subscribe_events().await?;
    let mut decoder = EventsDecoder::<Runtime>::new(client.chain_client().metadata().clone());
    decoder.with_bounty();
    let mut sub = EventSubscription::<Runtime>::new(sub, decoder);
